...
// ── FIREBASE CONFIG (loaded from .env — see .env.example) ───
...
const exportCSV = (trips, filename) => {
  ...
  const headers = ["Date","Lorry","Trip#","Revenue", ...allKeys.map(k => k.charAt(0).toUpperCase()+k.slice(1)), "Total Expenses","Profit","Status"];
  ...
  const rows = trips.map(t => {
    ...
    return [t.date, t.lorry, t.tripNumber, t.revenue, ...fixedVals, ...customVals, t.totalExpenses, t.profit, t.status];
  });
...
const exportPDF = (trips, title) => {
  ...
  autoTable(doc, {
    startY: 90,
    head: [["Date","Lorry","Trip#","Revenue", ...expCols, "Total Exp","Profit","Status"]],
    body: trips.map(t => {
...
function TripForm({ initial, onSave, onCancel }) {
  ...
  const handleSubmit = async () => {
    if (!form.date || !form.tripNumber || !form.revenue) {
      alert("Date, trip number, and revenue are required."); return;
    }
...
function TripsPage({ trips }) {
  ...
  const filtered = useMemo(() => trips.filter(t => {
    ...
    if (search) {
      ...
      return t.tripNumber?.toString().includes(s) || t.lorry?.toLowerCase().includes(s) || t.date?.includes(s);
    }
    return true;
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add New Trip" wide>
        <TripForm onSave={handleAdd} onCancel={() => setAddOpen(false)} />
      </Modal>
      <Modal open={!!editTrip} onClose={() => setEditTrip(null)} title="Edit Trip" wide>
        {editTrip && <TripForm initial={editTrip} onSave={handleEdit} onCancel={() => setEditTrip(null)} />}
      </Modal>
          ...
          {navItems.map(n => (
            <button key={n.id} onClick={() => setPage(n.id)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                page === n.id ? "bg-emerald-600 text-white shadow-md shadow-emerald-600/20" : "text-slate-600 hover:bg-slate-50"
              }`}>
            ...
            {navItems.map(n => (
              <button key={n.id} onClick={() => setPage(n.id)}
                className={`flex-1 flex flex-col items-center py-2.5 text-xs font-semibold transition-colors ${
                  page === n.id ? "text-emerald-600" : "text-slate-400 hover:text-slate-600"
                }`}>
...
function AppInner() {
  ...
  useEffect(() => {
    if (!user) return;
    return tripService.subscribe(setTrips);
  }, [user]);
