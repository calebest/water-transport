        ...
        <div className="table-scroll-container hidden md:block">
          <table className="w-full min-w-[1100px] text-sm">
            <thead className="bg-slate-50/50">
              <tr>
                {["Vehicle", "Trip #", "Location", "Revenue", "Expenses", "Profit", "Deductions", "Payable", "Status", ""].map(h => (
                  <th key={h} className="px-5 py-4 text-left text-[11px] font-black uppercase tracking-wider text-slate-400">{h}</th>
                ))}
              </tr>
            </thead>
                ...
                return (
                  <tr key={t.id}
                    className={`group/row transition-all duration-300 ${
                      ...
                      "bg-white hover:bg-slate-50 hover:shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] relative z-10 hover:z-20"
                    }`}
                  >
                    ...
                    <td className="px-5 py-4 text-slate-500 font-bold whitespace-nowrap">#{t.tripNumber}</td>
                    <td className="px-5 py-4 text-slate-700 font-medium truncate max-w-[150px]">{t.location || "N/A"}</td>
                    <td className="px-5 py-4 font-black text-blue-600 whitespace-nowrap">{fmt(financials.revenue)}</td>
                    <td className="px-5 py-4 font-black text-rose-500 whitespace-nowrap">{fmt(financials.operatingExpenses)}</td>
                    <td className="px-5 py-4 whitespace-nowrap">
                      <span className={`font-black ${financials.operatingProfit >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
                        {fmt(financials.operatingProfit)}
                      </span>
                    </td>
                    <td className="px-5 py-4 font-black text-amber-600 whitespace-nowrap">{fmt(financials.totalDeductions)}</td>
                    <td className="px-5 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 rounded-md text-xs font-black uppercase tracking-wider ${financials.netPayable >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
                      ...
                      <div className="flex flex-col gap-1.5">
                        {isAdmin && !isRejected && !isPending ? (
                          <select
                            ...
                            disabled={markingPaid === t.id}
                            onChange={e => onStatusChange(t, e.target.value)}
                            className={`rounded-xl border px-3 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 disabled:opacity-60 cursor-pointer transition-colors
                          ...
                          <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${paymentSelectClass(t.status)}`}>
                            {t.status}
                          </span>
                        )}
                        ...
                        {approval && APPROVAL_BADGE[approval] && (
                          <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${
                             approval === "rejected" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"
                          }`}>
                            {APPROVAL_BADGE[approval].label}
                          </span>
                        )}
                    ...
                    {/* Actions column - Fades in on hover */}
                    <td className="px-5 py-4 whitespace-nowrap text-right">
                      <div className="flex justify-end gap-1 opacity-100 md:opacity-0 md:group-hover/row:opacity-100 transition-opacity">
                        {/* Print */}
                        {!isRejected && (
                          <button onClick={() => exportVoucher(t)}
                        ...
                        {isAdmin && isPendingEdit && onApprove && onReject && (
                          <div className="flex gap-1 bg-amber-50 rounded-xl p-1 border border-amber-100 ml-2">
                            <button onClick={() => onApprove(t)}
                              className="p-1 rounded-lg hover:bg-emerald-100 text-emerald-600 transition-colors"
                              title="Apply Edit"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></button>
                            <button onClick={() => onReject(t)}
                              className="p-1 rounded-lg hover:bg-rose-100 text-rose-600 transition-colors"
                              title="Discard Edit"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>
                          </div>
                        )}
                      </div>
                    </td>
