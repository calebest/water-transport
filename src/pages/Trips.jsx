        ...
        {groupedTrips.length === 0 ? (
          ...
          <>
            ...
            <div className="flex justify-end mb-3">
              <CollapseAllButton groups={groupedTrips} />
            </div>
...
// ─── Collapse All Button (controls all TripGroups via a shared ref map) ────────
function CollapseAllButton() {
  const [allExpanded, setAllExpanded] = useState(true);
  return (
    <button
      onClick={() => {
        // Dispatch a custom event that TripGroups listen to
        window.dispatchEvent(new CustomEvent("trips:setAllExpanded", { detail: !allExpanded }));
        setAllExpanded(!allExpanded);
      }}
      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors shadow-sm"
    >
      <svg className={`w-3.5 h-3.5 transition-transform ${allExpanded ? '' : 'rotate-180'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
...
export function TripGroup({ group, isAdmin, onEdit, onDel, onStatusChange, markingPaid, onApprove, onReject, userId, canAddTrips, onOpenTripReview }) {
  ...
  // Listen to global collapse/expand events
  useEffect(() => {
    const handler = (e) => setExpanded(e.detail);
    window.addEventListener("trips:setAllExpanded", handler);
    return () => window.removeEventListener("trips:setAllExpanded", handler);
  }, []);
  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden">
      {/* Group Header — always white */}
      <div
        className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 cursor-pointer hover:bg-slate-50/70 transition-colors border-b border-slate-100"
        onClick={() => setExpanded(!expanded)}
      >
        {/* Left: Date + count */}
        <div className="flex items-center gap-3">
          <div className={`flex items-center justify-center w-7 h-7 rounded-full transition-colors ${expanded ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-500"}`}>
            <svg className={`w-3.5 h-3.5 transition-transform duration-200 ${expanded ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </div>
          <div>
            <h3 className="text-base font-black tracking-tight text-slate-800">{group.date}</h3>
            <p className="text-[11px] font-semibold text-slate-400">{group.trips.length} trip{group.trips.length !== 1 ? 's' : ''}</p>
          </div>
        </div>

        {/* Right: Summary pills — 2×2 grid on mobile, row on sm+ */}
        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 sm:gap-1.5 sm:items-center">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-100">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Rev</span>
            <span className="text-xs font-black text-blue-600">{fmt(group.summary.revenue)}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-100">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Exp</span>
            <span className="text-xs font-black text-rose-500">{fmt(group.summary.operatingExpenses)}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-100">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Ded</span>
            <span className="text-xs font-black text-amber-500">{fmt(group.summary.deductions)}</span>
          </div>
          <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border ${
            group.summary.operatingProfit >= 0 ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100"
          }`}>
            <span className={`text-[10px] font-bold uppercase tracking-wider ${ group.summary.operatingProfit >= 0 ? "text-emerald-500" : "text-rose-400"}`}>Profit</span>
            <span className={`text-xs font-black ${ group.summary.operatingProfit >= 0 ? "text-emerald-600" : "text-rose-600"}`}>{fmt(group.summary.operatingProfit)}</span>
          </div>
        </div>
      </div>
